...
test('Email investigation UI uses a focused input page, modal processing, and dedicated forensic result page', () => {
  ...
  const emailCss = read('assets/css/pages/email-investigation.css');
  const workspaceCss = read('assets/css/pages/analysis-workspace.css');
  const modalCss = read('assets/css/pages/analysis-progress-modal.css');
  const linkPage = read('link-check.html');
  ...
  assert.doesNotMatch(page, /LIVE FORENSIC ENGINE|SIH26106/u);
  assert.match(page, /<dialog[^>]+analysis-progress-modal/u);
  assert.match(page, /id="analysisProgressEvents"/u);
  assert.doesNotMatch(page, /id="emailInvestigationResult"/u);
  assert.match(resultPage, /id="emailInvestigationResult"/u);
  assert.match(resultPage, /email-report-pdf\.js/u);
  assert.match(resultPage, /noindex, nofollow/u);
  assert.match(script, /\/api\/v1\/gateway\/email\/analyze-text/u);
  assert.match(script, /\/api\/v1\/gateway\/email\/analyze-eml/u);
  assert.match(script, /\/phishing-result\?scan_id=/u);
  ...
  assert.match(progress, /showModal/u);
  assert.match(progress, /dismiss/u);
  assert.match(emailCss, /\.email-help-button\s*\{[^}]*width:\s*14px/isu);
  assert.match(workspaceCss, /body\.vt-tool-page\.vt-analysis-page \.btn \{ min-height: 38px/iu);
  assert.match(workspaceCss, /\.analysis-module-nav a \{[^}]*min-height: 30px/isu);
  assert.match(modalCss, /dialog\.analysis-progress-modal::backdrop/iu);
  assert.match(modalCss, /data-state="running"/u);
  assert.match(linkPage, /<dialog[^>]+analysis-progress-modal/u);
  assert.doesNotMatch(linkPage, /class="analysis-activity"/u);
});
