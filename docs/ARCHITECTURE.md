...
The executable runtime currently resolves detector aliases from server-only configuration and normalizes provider responses in shared services. This is current-state behavior, not a qualified scientific model registry. The target architecture requires immutable model, processor, label-map, benchmark, and calibrator records before inference; see `output/pdfs/VeriTrust_Hugging_Face_Model_Research_and_Acquisition_Guide.pdf` for the implementation handoff. The guide is a retained research artifact, not a deployed runtime dependency.
...
Future model workers should be dedicated by scientific role. They must not silently substitute a face-manipulation model for full-frame synthetic-image evidence, guess unknown labels, or use provider fallback as an evidence-equivalent replacement.
