...
values
  ...
  ('swift', 'link', 'VeriTrust Swift', 'hf-inference', 'CrabInHoney/urlbert-tiny-v4-malicious-url-classifier', true, null),
  ('sentinel', 'link', 'VeriTrust Sentinel', 'hf-inference', 'ilkayO/modernbert-phishing-detector', false, 'swift')
...
as $$
  select coalesce((
    select (coalesce(u.deepfake_count, 0) + coalesce(u.phishing_count, 0) + coalesce(u.link_count, 0) + coalesce(u.api_count, 0)) < p.daily_scan_limit
...
as $$
begin
  insert into public.user_usage_daily (org_id, user_id, usage_date, deepfake_count, phishing_count, link_count, api_count)
  values (
    target_org_id,
    target_user_id,
    current_date,
...
create table public.api_rate_limits (
  ...
  updated_at timestamptz not null default now(),
  unique (identity_type, identity_hash, endpoint, window_date)
);
...
create table public.api_keys (
  ...
  scopes jsonb not null default '["deepfake:scan","phishing:scan","link:scan","usage:read"]'::jsonb,
...
create index api_rate_limits_endpoint_window_idx on public.api_rate_limits(endpoint, window_date);
...
revoke execute on function public.consume_api_rate_limit(text, text, text, integer, jsonb) from public, anon, authenticated;
...
grant execute on function public.consume_api_rate_limit(text, text, text, integer, jsonb) to service_role;
grant all on table public.api_rate_limits to service_role;

commit;

-- Existing database upgrade note:
-- If your Supabase project was created before Critical risk support, run this once:
-- alter type public.risk_level add value if not exists 'critical';
alter type public.scan_type add value if not exists 'link';

-- Prompt 3 developer API upgrade.
-- Safe to run on an existing VeriTrust database after reviewing in Supabase SQL editor.
alter table public.api_keys
  add column if not exists user_id uuid references auth.users(id) on delete set null,
  add column if not exists masked_key text,
  add column if not exists usage_limit_daily integer not null default 100,
...
alter table public.api_keys
  alter column scopes set default '["deepfake:scan","phishing:scan","link:scan","usage:read"]'::jsonb;

update public.api_keys
set scopes = (
  case
    when jsonb_typeof(scopes) = 'array' and not (scopes ? 'link:scan') then scopes || '["link:scan"]'::jsonb
    else scopes
  end
)
...
set
  ...
  masked_key = coalesce(masked_key, key_prefix || '...'),
  usage_limit_daily = coalesce(usage_limit_daily, 100)
where user_id is null
   or masked_key is null
   or usage_limit_daily is null;
...
create index if not exists api_usage_events_api_key_id_created_at_idx on public.api_usage_events(api_key_id, created_at desc);
create index if not exists api_usage_events_user_id_created_at_idx on public.api_usage_events(user_id, created_at desc);
create index if not exists api_usage_events_org_id_created_at_idx on public.api_usage_events(org_id, created_at desc);

alter table public.api_usage_events enable row level security;

drop policy if exists "api_usage_events_select_owner" on public.api_usage_events;
create policy "api_usage_events_select_owner"
...
-- API key creation, revocation, verification, and key-hash reads are handled by
-- Vercel serverless functions with the Supabase service role. Direct frontend
-- reads should not expose key_hash.
