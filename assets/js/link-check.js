...
function renderResult(data) {
  ...
  const degraded = model.degraded === true || model.inference_mode === 'local_rules';
  const confidence = result.confidence == null ? 'Unavailable' : formatPercent(result.confidence);
  const score = formatPercent(result.link_score);
  const fallback = Boolean(model.fallback_used || model.fallback_from || degraded);
  ...
  target.innerHTML = `
    ...
    ${degraded ? '<p class="fallback-notice"><strong>Pattern-only degraded result.</strong> Hosted inference was unavailable; manually verify this result.</p>' : ''}
    <div class="score-meter"><span class="${riskClass(result)}" style="width:${Math.max(2, Number.parseInt(score, 10) || 0)}%"></span></div>
    ...
    ${(data.warnings || (data.warning ? [data.warning] : [])).map((warning) => `<p class="fallback-notice"><strong>${escapeHtml(warning.code || 'Warning')}:</strong> ${escapeHtml(warning.message || '')}</p>`).join('')}
    ${Array.isArray(result.per_url_results) ? `<div class="result-section"><h3>Per-URL results</h3>${result.per_url_results.map((item) => item.ok ? `<article class="signal-item"><div><strong>${escapeHtml(item.url)}</strong><p>${escapeHtml(item.result?.label || 'Unknown')} - ${escapeHtml(item.result?.risk_level || 'Unknown')} risk - score ${escapeHtml(formatPercent(item.result?.link_score))}</p></div></article>` : `<article class="signal-item"><div><strong>${escapeHtml(item.url)}</strong><p>Error: ${escapeHtml(item.error?.message || 'Unable to analyze this URL.')}</p></div></article>`).join('')}</div>` : ''}
    <div class="result-section">
      <h3>Indicators</h3>
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    qs('#linkSubmit')?.setAttribute('disabled', '');
