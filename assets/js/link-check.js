...
function renderResult(data) {
  ...
  target.innerHTML = `
    ...
    ${fallback ? `<p class="fallback-notice">Fallback used: ${escapeHtml(model.fallback_from || modelMeta.fallback_from || 'selected model')} to ${escapeHtml(model.name || 'VeriTrust Swift')}.</p>` : ''}
    <div class="result-section">
      <h3>Indicators</h3>
...
function renderError(message) {
  renderResult({
    ...
    result: {
      ...
      indicators: [],
      extracted: {},
      disclaimer: 'AI-assisted result. Verify suspicious links through official channels before opening or submitting information.',
    },
  });
}
