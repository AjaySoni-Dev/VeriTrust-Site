...
function absoluteCropUrl(url) {
  ...
  if (/^data:image\/(?:jpeg|png|webp|bmp);base64,[A-Za-z0-9+/=\s]+$/i.test(value)) return value;
  try {
    const base = new URL(config.cropOutputBaseUrl);
...
function rememberOriginalImageDataUrl(file) {
  ...
  reader.addEventListener('load', () => {
    state.originalImageDataUrl = typeof reader.result === 'string' ? reader.result : '';
  });
...
async function ensureOriginalImageDataUrl() {
  if (!state.originalImageDataUrl && state.originalImageFile) {
    state.originalImageDataUrl = await fileToDataUrl(state.originalImageFile);
  }
...
async function urlToFile(url, filename) {
  ...
  if (!safeUrl) throw new Error('The crop service returned an invalid image URL.');
  ...
  if (declaredSize > config.maxImageBytes) throw new Error('The cropped image is too large.');
  const blob = await response.blob();
  if (!blob.size || blob.size > config.maxImageBytes || !['image/jpeg', 'image/png', 'image/webp', 'image/bmp'].includes(blob.type)) {
    throw new Error('The crop service returned an unsupported image.');
  }
...
function cropSource(face) {
  if (!face) return '';
  return absoluteCropUrl(face.data_url || face.crop_url);
}
...
function renderSelectedCropPreview(index) {
  ...
  if (!cropUrl) {
    wrap.classList.remove('has-crop');
    wrap.textContent = 'The crop service returned an invalid image URL. The full image will be checked.';
    return;
  }
  const faceButtons = state.cropFaces.map((item, itemIndex) => `
    <button class="${itemIndex === index ? 'active' : ''}" type="button" data-crop-index="${itemIndex}">
      Face ${escapeHtml(item.face_index || itemIndex + 1)}
    </button>
  `).join('');
  ...
  wrap.innerHTML = `
    <div class="crop-preview-head">
      ...
      <div class="crop-preview-actions">
        <a id="downloadCrop" download="VeriTrust-face-crop.jpg" href="${escapeHtml(cropUrl)}">Download</a>
        ${annotatedUrl ? `<a target="_blank" rel="noopener noreferrer" href="${escapeHtml(annotatedUrl)}">View marked image</a>` : ''}
      </div>
    </div>
    <div class="crop-preview-grid${annotatedUrl ? '' : ' single'}">
      <figure>
        <img src="${escapeHtml(cropUrl)}" alt="Selected cropped face">
        <figcaption>Focused face</figcaption>
      </figure>
      ${annotatedUrl ? `
        <figure>
          <img src="${escapeHtml(annotatedUrl)}" alt="Annotated original image">
...
async function setSelectedCropFile(index, updatePreview = true) {
  ...
  const source = cropSource(face);
  if (!source) throw new Error('The crop service returned an invalid image URL.');
  state.selectedDeepfakeFile = source.startsWith('data:image/')
    ? dataUrlToFile(source, filename)
    : await urlToFile(source, filename);
