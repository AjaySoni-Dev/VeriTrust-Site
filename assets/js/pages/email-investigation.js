  ...
  function renderResult(payload) {
    ...
    target.innerHTML = `
      ...
      <details class="analysis-evidence-details"><summary>Evidence details and limitations <span>${limitations.length} recorded limitations</span></summary><div class="email-evidence-grid">
        ...
        <article class="email-evidence-card"><header><h3>${helpLabel('Delivery route', HELP.infrastructure)}</h3><span>${escapeHtml(friendlyStatus(infrastructureSummary.state || 'Approximate'))}</span></header>${evidenceList(infrastructure, 'No eligible public mail-server step was available. This does not mean the email had no delivery route.', (item) => ({ label: `Mail server ${Number(item.hop_index ?? 0) + 1}`, detail: `${item.host || item.ip_address || 'Unknown server'}${item.asn_org ? ` - ${item.asn_org}` : ''}${item.country ? ` - ${item.country}` : ''}${item.trust_level ? ` - ${friendlyStatus(item.trust_level)}` : ''}`, help: HELP.infrastructure }))}${infrastructureSummary.earliest_reliable_public_node ? `<p class="email-retention-note"><strong>Trusted-boundary public node:</strong> ${escapeHtml(infrastructureSummary.earliest_reliable_public_node.host || infrastructureSummary.earliest_reliable_public_node.ip_address || 'Recorded')}.</p>` : ''}<p class="email-retention-note">${escapeHtml(infrastructureSummary.wording || 'Any location shown belongs to network infrastructure, not a person.')}</p></article>
