(function emailInvestigationPage(global) {
  ...
  function setBusy(busy) {
    ...
    button.textContent = busy ? 'Analyzing…' : state.mode === 'eml' ? 'Analyze original email' : 'Analyze email';
  ...
  function setMode(mode, focusPanel = false) {
    ...
    const submitHint = one('#emailSubmitHint');
    if (submitHint) submitHint.textContent = rawEvidence
      ? 'Headers, authentication, attachments, links, and recorded delivery infrastructure will be checked when available.'
      : 'Content and visible links will be analyzed.';
  ...
  function renderResult(payload) {
      ...
      <article class="email-result-hero" data-state="${specialistState}">
        <div>
          ...
          <div class="email-result-actions">
            <a class="btn btn-primary" href="/phishing">New investigation</a>
    ...
    one('[data-upgrade-evidence]', target)?.addEventListener('click', (event) => {
      const parentScanId = event.currentTarget.dataset.parentScan || payload.scan_id || null;
      const query = new URLSearchParams({ mode: 'eml' });
      if (parentScanId) query.set('parent_scan_id', parentScanId);
      global.location.assign(`/phishing?${query.toString()}`);
    });
    one('[data-download-email-pdf]', target)?.addEventListener('click', async (event) => {
      ...
      try {
        ...
        setError(`${error.message} Try again or reload the report.`);
        global.setTimeout(() => { button.textContent = originalLabel; button.disabled = false; }, 3200);
      }
    });
    if (!document.body?.classList?.contains('vt-email-result-page')) shell.scrollIntoView({ behavior: global.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth', block: 'start' });
  }
  ...
  function initResultPage() {
    ...
    if (!savedScanId) {
      const error = new Error('No investigation report was specified.');
      error.code = 'EMAIL_SCAN_ID_REQUIRED';
      renderFailure(error);
      setStatus('Open a saved investigation or start a new email analysis.');
      return;
    }
  ...
  function initInputPage(form) {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      setError('');
      setBusy(true);
      analysisProgress.begin(state.mode === 'eml' ? 'Validating the original email evidence.' : 'Validating the submitted email content.');
      setStatus('');
      try {
        const payload = await requestInvestigation();
        if (!payload.scan_id) throw new Error('The completed analysis did not return a report identifier.');
        if (!payload.evidence && payload.status === 'processing') {
          analysisProgress.finish(null, { pending: true });
          global.location.assign(`/phishing-result?scan_id=${encodeURIComponent(payload.scan_id)}`);
          return;
        }
        ...
        analysisProgress.finish(null, { title: 'Analysis complete', message: 'Opening the investigation report.' });
        global.location.assign(`/phishing-result?scan_id=${encodeURIComponent(payload.scan_id)}`);
      } catch (error) {
        analysisProgress.finish(error);
        const guidance = failureGuidance(error);
        setError(`${error.message} ${guidance.detail}`);
        setStatus('The investigation was not completed.');
      } finally {
        setBusy(false);
      }
    });

    const requestedMode = queryParameter('mode');
    const parentScanId = queryParameter('parent_scan_id');
    if (parentScanId && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(parentScanId)) state.parentScanId = parentScanId;
    setMode(requestedMode === 'eml' ? 'eml' : 'text');
  }

  function init() {
    if (document.body?.classList?.contains('vt-email-result-page')) {
      initResultPage();
      return;
    }
