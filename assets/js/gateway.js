(function initGateway(global) {
  ...
  function render(data) {
    ...
    elements.evidence.replaceChildren(...(data.evidence || []).map((item) => { const article=document.createElement('article'); const header=document.createElement('header'); const strong=document.createElement('strong'); strong.textContent=item.model; const status=document.createElement('span'); status.textContent=item.status; header.append(strong,status); const detail=document.createElement('p'); detail.textContent=item.score===null||item.score===undefined?'No score available':`Score ${Math.round(Number(item.score)*100)}% · ${item.verdict} · ${item.model_version}`; article.append(header,detail); return article; }));
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      const token=await api.getAccessToken(); if(!token)throw new Error('Sign in before submitting a gateway scan.');
  ...
  api.getAccessToken().then((token)=>{elements.auth.hidden=Boolean(token);form.querySelectorAll('input,textarea,select,button').forEach((item)=>item.disabled=!token);if(token)refreshHistory();});
})(window);
