...
const runtimeConfig = window.VeriTrust_CONFIG || {};
let config = {
  ...DEFAULT_CONFIG,
  ...runtimeConfig,
...
async function authHeaders() {
  ...
  return window.VeriTrustSupabase.authHeaders();
}
...
async function boundedThumbnail(file, maximumDimension = 900) {
  ...
  try {
    ...
    return dataUrl.length <= 700_000 ? dataUrl : canvas.toDataURL('image/jpeg', 0.5);
  } finally {
    bitmap.close();
  }
}

function rememberOriginalImageDataUrl(file) {
  state.originalImageDataUrl = '';
  state.thumbnailPromise = file ? boundedThumbnail(file).then((value) => {
    state.originalImageDataUrl = value;
    return value;
  }).catch(() => '') : Promise.resolve('');
}

async function ensureOriginalImageDataUrl() {
  if (!state.originalImageDataUrl && state.thumbnailPromise) state.originalImageDataUrl = await state.thumbnailPromise;
  return state.originalImageDataUrl;
}
...
function renderImagePreview(file) {
  ...
  image.src = state.originalImageUrl;
  rememberOriginalImageDataUrl(file);
...
function downloadJsonReport(data) {
  ...
  if (isDeepfake && window.VeriTrustReporting?.sanitizeVisualsForJson) {
    const visuals = window.VeriTrustReporting.sanitizeVisualsForJson(buildDeepfakeReportVisuals());
    if (visuals.available) report.visuals = visuals;
  }
...
function buildDeepfakeReportVisuals() {
  ...
  const analyzedImageUrl = croppedImageUrl || state.originalImageDataUrl || state.originalImageUrl || qs('#imagePreview')?.src || '';
  return {
    available: Boolean(croppedImageUrl || annotatedImageUrl || analyzedImageUrl),
    selected_face_index: selectedFace ? state.selectedCropIndex : null,
...
async function cropImage() {
  ...
  throw new Error('Optional face preprocessing is disabled until a consented server-side processor is configured. Turn off automatic cropping to analyze the original image.');
}
...
function renderTechnicalError(targetId, error) {
  const target = document.getElementById(targetId);
  if (!target) return;
  target.classList.remove('result-empty', 'result-loading', 'result-ready');
  target.classList.add('result-error');
  target.replaceChildren();
  const heading = document.createElement('strong');
  heading.textContent = 'Analysis could not be completed';
  const detail = document.createElement('p');
  detail.textContent = error?.message || 'Please retry the request.';
  const metadata = document.createElement('p');
  metadata.className = 'result-muted';
  metadata.textContent = [error?.code, error?.requestId ? `Request ${error.requestId}` : ''].filter(Boolean).join(' - ');
  target.append(heading, detail, metadata);
}
  ...
  if (autoCrop && (!state.selectedDeepfakeFile || state.selectedDeepfakeFile === state.originalImageFile)) {
    try {
      ...
      if (cropResults) {
        ...
        cropResults.textContent = error.message;
      }
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    const readyConfig = await window.VeriTrust_CONFIG_READY;
    config = { ...DEFAULT_CONFIG, ...readyConfig, api: { ...DEFAULT_CONFIG.api, ...(readyConfig.api || {}) } };
    apiConfig = config.api;
  } catch {
    qsa('form input, form button, form textarea, form select').forEach((control) => { control.disabled = true; });
    setLog('Service configuration unavailable. Reload to retry.');
    return;
  }
  bindModules();
  ...
  qs('#deepfakeForm')?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      renderTechnicalError('deepfakeResult', error);
      setLog(error.message);
    } finally {
      setLoading(button, false);
    }
  ...
  qs('#phishingForm')?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      renderTechnicalError('phishingResult', error);
      setLog(error.message);
    } finally {
      setLoading(button, false);
    }
