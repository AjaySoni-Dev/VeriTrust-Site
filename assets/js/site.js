document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateAuthNavigation = async () => {
    ...
    const ensureMobileMenuActions = () => {
      ...
      let logoutAction = wrap.querySelector('[data-mobile-logout-action]');
      if (isDashboard) {
        if (!logoutAction) {
          logoutAction = document.createElement('a');
          logoutAction.href = '#sign-out';
          logoutAction.textContent = 'Logout';
          logoutAction.dataset.mobileLogoutAction = 'true';
          wrap.appendChild(logoutAction);
        }
        bindLogout(logoutAction);
    ...
    const removeMobileMenuActions = () => {
      toolHeaderLinks?.querySelector('.tool-header-mobile-actions')?.remove();
    };
