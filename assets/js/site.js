document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateAuthNavigation = async () => {
    ...
    const authLinks = document.querySelectorAll('.tool-header-login, .login-link, .nav-actions a[href="/auth"], .nav-actions a[href="auth.html"]');
    const dashboardLinks = document.querySelectorAll('.tool-header-dashboard, a[href="/dashboard"], a[href="dashboard.html"]');
    ...
    const ensureMobileMenuActions = (sessionActive = false) => {
      ...
      const hasDashboardNavLink = Boolean(toolHeaderLinks.querySelector(':scope > a[href="/dashboard"], :scope > a[href="dashboard.html"]'));
  ...
  updateAuthNavigation().catch(() => {
    document.body.classList.remove('vt-authenticated');
    document.body.classList.add('vt-session-unknown');
  });
