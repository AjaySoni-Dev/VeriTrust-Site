...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const scanId = await createScanRecord(context, {
      ...
      retainText: Boolean(body.retain_text),
    ...
    try {
      const { payload, modelRuns } = await runPhishingDetection({ text, modelKey, scanId, context, createdAt });
      await completeScanRecord(scanId, payload, modelRuns);
      await recordBillableUsage(context, {
        source: 'web', scanType: 'phishing', endpoint: '/api/phishing', requestId: req.requestId,
        reservationId: entitlement.decision?.reservation_id,
        metadata: { scan_id: scanId, model_key: modelKey },
      });
      sendJson(res, 200, payload);
    } catch (error) {
      await releaseQuotaReservation(entitlement.decision?.reservation_id, error.code || 'PHISHING_SCAN_FAILED');
      try { await failScanRecord(scanId, String(error.code || 'PHISHING_SCAN_FAILED')); }
      catch (cleanupError) { error.cleanup_error = cleanupError.code || 'SCAN_FAILURE_PERSIST_FAILED'; }
      throw error;
    }
  } catch (error) {
