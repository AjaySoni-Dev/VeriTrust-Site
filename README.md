...
The API also accepts `HF_ACCESS_TOKEN` instead of `HF_TOKEN`. `ADMIN` is optional and enables private diagnostics on `/api/health`. `CONTENT_HMAC` must contain at least 32 random bytes; `WEBHOOK_ENCRYPTION` must be a 32-byte key encoded as 64 hexadecimal characters; `DISPATCH` must be the same 32+ character random value stored by migration 003 in Supabase Vault. The longer `VERITRUST_*` names remain supported as aliases, but the short names match the configured Vercel project. Never expose any of these server credentials in browser JavaScript.
