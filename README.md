...
| Link Intelligence | `swift` | VeriTrust Swift | `hf-inference` | `CrabInHoney/urlbert-tiny-v4-malicious-url-classifier` |
| Link Intelligence | `sentinel` | VeriTrust Sentinel | `hf-inference` | `ilkayO/modernbert-phishing-detector` |
...
If `model` is missing, VeriTrust uses `swift`. If `sentinel` is unavailable through the current Hugging Face provider, the scan falls back to `swift` and includes fallback metadata in the response.
...
- Sentinel may fall back to Swift if the selected Hugging Face model is unavailable through the current provider.
