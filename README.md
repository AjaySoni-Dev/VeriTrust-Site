...
Create an ignored local environment file and populate only the values needed for local work:
...
- The target model stack requires pinned artifacts, role-specific adapters, benchmark evidence, calibration, legal review, and controlled rollout; the current aliases alone do not provide those gates.
...
- implementation of the approved model-acquisition guide through dedicated workers and immutable model registry records
...
- Do not commit model weights, ad hoc model conversions, provider-response captures, or model tokens; acquire approved artifacts into an immutable external registry or private mirror only after the documented review gates.
