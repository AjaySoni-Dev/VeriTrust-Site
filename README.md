...
Only the maintained Node.js serverless routes under `api/` are deployable; the unauthenticated legacy PHP implementation has been removed.
...
| Authentication UI | Active with Supabase | Login, signup, password recovery, session checks, and dashboard access use Supabase Auth when configured |
...
| Link Intelligence | `swift` | VeriTrust Swift | `hf-inference` | `kmack/malicious-url-detection` |
| Link Intelligence | `sentinel` | VeriTrust Sentinel | Locked | Coming soon |
...
|   |-- health.js          # Minimal public liveness endpoint
|   |-- ready.js           # Authenticated owner/admin readiness endpoint
...
|   `-- js/
|       |-- core/          # Validated configuration and auth routing clients
|       |-- config.js      # Asynchronous browser configuration bootstrap
...
|-- lib/
|   |-- security.js        # Origin, CSRF, request ID, redaction, and bounded-fetch controls
|   |-- session.js         # HttpOnly session, recovery, CSRF, and device lifecycle
...
|-- auth.html              # Server-session login, signup, and recovery interface
...
HF_TOKEN=
VERITRUST_SITE_URL=https://veritrustlab.in
VERITRUST_ALLOWED_ORIGINS=https://veritrustlab.in
VERITRUST_SESSION_SECRET=
VERITRUST_API_KEY_PEPPER=
VERITRUST_EXTERNAL_API_ENABLED=false
VERITRUST_BILLING_ENABLED=false
```

The API also accepts `HF_ACCESS_TOKEN` instead of `HF_TOKEN`. Public liveness is available at `/health/live`; authenticated owner/admin readiness is available at `/health/ready`. Never expose service-role, provider, session, or API-key pepper secrets in browser JavaScript.
...
Raw API keys are returned only once when created. VeriTrust stores a non-secret `public_id`, a versioned HMAC digest protected by a server pepper, a display hint, ownership, scopes, status, timestamps, and usage limits. Pre-remediation keys are revoked by migration `202607100002`.
...
Returns minimal public liveness only. Detailed readiness requires an authenticated owner/admin session and recent authentication at `/health/ready`.
...
|---|---|---:|---|
| `image` | File | Yes | JPG, PNG, or WEBP image with a matching file signature |
...
Browser-side configuration is loaded asynchronously from the same-origin `/api/client-config` contract by:

```text
assets/js/core/config-client.js
```

No hard-coded preprocessing provider exists. Optional preprocessing stays disabled unless an approved server-side provider, display name, and consent version are configured. Crop failure never silently changes the analyzed image.
...
- revalidated caching for non-hashed `/assets/*`
- CSP, HSTS, COOP/CORP, content-type, referrer, permissions, and frame protections
...
HF_TOKEN=
VERITRUST_SITE_URL=https://veritrustlab.in
VERITRUST_ALLOWED_ORIGINS=https://veritrustlab.in,https://www.veritrustlab.in
VERITRUST_SESSION_SECRET=
VERITRUST_API_KEY_PEPPER=
VERITRUST_EXTERNAL_API_ENABLED=false
VERITRUST_BILLING_ENABLED=false
...
Apply `supabase/migrations/202607100001_baseline.sql` through `202607100003_scan_lifecycle.sql` with the migration runner. Keep external API and billing flags disabled until their contract, replay, concurrency, and load tests pass.

Configure secrets only in Vercel Environment Variables or local ignored `.env` files. Never store provider, Supabase service-role, session, or pepper values in repository files.
...
- Never expose `SUPABASE_SERVICE_ROLE_KEY`, session secrets, peppers, or provider credentials in browser JavaScript.
