...
The API also accepts `HF_ACCESS_TOKEN` instead of `HF_TOKEN`. `VERITRUST_ADMIN_SECRET` is optional and enables private diagnostics on `/api/health`. The gateway HMAC key must be at least 32 random bytes; the webhook encryption key must be 64 hexadecimal characters. `VERITRUST_GATEWAY_DISPATCH_SECRET` must be the same 32+ character random value stored by migration 003 in Supabase Vault. Never expose any of these server credentials in browser JavaScript.
...
1. Generate one random 32+ character dispatch secret and set it as `VERITRUST_GATEWAY_DISPATCH_SECRET` in Vercel for Production, Preview, and Development as appropriate.
