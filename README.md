...
| Production observability | Planned | Logging, monitoring, rate limiting, and analytics are not finalized |
...
|   |-- health.js          # Runtime and token readiness endpoint
...
Create a local environment file or configure the variable in Vercel:

```env
HF_ACCESS_TOKEN=hf_your_token_here
```

The API also checks `HF_TOKEN` as a fallback name.
...
Returns service metadata, configured model display names, runtime name, and whether the Hugging Face token is available.
...
- authentication pages are UI-only and do not connect to a real auth backend
- no persistent user dashboard or scan history is implemented
- no database is currently wired into this repository
- no production rate limiting, abuse protection, or billing layer is included
...
- add persistent scan history and user-owned workspaces
- introduce rate limiting and request abuse protection
...
| Check deployment health | Open `/api/health` after configuring the token |
...
- Do not commit `.env`, `.env.local`, or private token files.
...
- Add rate limiting before opening the API to public high-volume traffic.
