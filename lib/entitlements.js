...
async function billingSnapshot(contextOrOrgId) {
  ...
  const subscriptionRows = await tryService(`/rest/v1/organization_subscriptions?org_id=eq.${eq(orgId)}&select=*&order=created_at.desc&limit=1`, []);
  const usageRows = await tryService(`/rest/v1/usage_monthly?org_id=eq.${eq(orgId)}&month_start=eq.${currentMonth}&select=*`, []);
  const apiKeyCount = await countActiveApiKeys(orgId);
  const subscription = subscriptionRows?.[0] || null;
  const usage = usageRows?.[0] || {};
  const apiUsed = Number(usage.api_deepfake_count || 0) + Number(usage.api_phishing_count || 0) + Number(usage.api_usage_count || 0);
  const webUsed = Number(usage.web_deepfake_count || 0) + Number(usage.web_phishing_count || 0);
...
async function rpcDecision(context, options) {
  try {
    ...
    if (isMissingBillingObject(error)) return null;
    throw error;
  }
}
