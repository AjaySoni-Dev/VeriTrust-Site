...
async function runPreparedLinkBatch(orgId, scanId, artifacts, policy) {
  ...
  if (runnable.length) {
    ...
    const results = await linkAdapter.executeBatch(runnable.map((slot) => slot.artifact), {
      modelKey: 'swift',
      required: true,
      timeoutMs,
      allowProviderFallback: false,
  ...
  try {
    ...
    const [modelEvidence, linkEvidence] = await Promise.all([
      parserFailure
        ...
        : runPreparedAdapter(auth.organization.id, scanId, parentView, 'mailguard', phishingAdapter, policy, true),
