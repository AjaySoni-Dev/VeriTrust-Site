...
function failedModelEvidence(kind, modelKey, artifactId, error, required = true) {
  ...
  return {
    ...
    kind,
    modelKey,
    status: /TIMEOUT/u.test(String(error?.code || '')) ? 'timed_out' : 'failed',
    score: null,
    verdict: 'unknown',
    confidence: 'unknown',
...
async function analyzeEmail(input) {
  ...
  try {
    ...
    const modelTasks = [
      ...
      ...hydratedChildren.filter((child) => child.type === 'url').map((child) => ({
        kind: 'link',
        artifact: child,
        modelKey: 'swift',
        adapter: linkAdapter,
    ...
    const modelResults = await mapWithConcurrency(
      ...
      async (task) => task.parserFailure
        ? failedModelEvidence(task.kind, task.modelKey, task.artifact.id, task.parserFailure, true)
        : runPreparedAdapter(auth.organization.id, scanId, task.artifact, task.modelKey, task.adapter, policy, true),
    );
    const modelEvidence = modelResults[0];
    const childEvidence = new Map();
    modelTasks.slice(1).forEach((task, index) => childEvidence.set(task.artifact.id, modelResults[index + 1]));
