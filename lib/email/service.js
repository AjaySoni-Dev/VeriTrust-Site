...
function evidenceCompleteness(input) {
  ...
  const dimensions = {
    ...
    links: !urlChildren.length ? 'CHECKED' : (urlStates.every((state) => state === 'completed') ? 'CHECKED' : (urlStates.some((state) => state === 'completed') ? 'LIMITED' : 'LIMITED')),
  ...
  const level = checked >= 6 && unavailable <= 1 ? 'STRONG' : (checked >= 4 ? 'MODERATE' : 'LIMITED');
  return {
    level,
