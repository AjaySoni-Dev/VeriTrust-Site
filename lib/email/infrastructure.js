...
async function enrichInfrastructure(hops, provider = null) {
  if (!provider || typeof provider.lookup !== 'function') return { hops, state: 'UNAVAILABLE', provider: null };
  const enriched = [];
  for (const hop of hops) {
    ...
    try {
      ...
      enriched.push({
        ...
        provenance: { ...hop.provenance, provider_quality: result.quality || 'unknown' },
      });
    } catch (error) {
      enriched.push({ ...hop, provenance: { ...hop.provenance, enrichment_error: String(error.code || 'LOOKUP_FAILED') } });
    }
  }
  return { hops: enriched, state: 'COMPLETED', provider: provider.name || 'configured' };
}
