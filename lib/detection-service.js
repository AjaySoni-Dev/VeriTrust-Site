...
async function runMailGuard(text, timeoutMs = 25000) {
  const modelKey = 'mailguard';
  const model = PHISHING_MODELS[modelKey];
  const result = await hfJsonInference(model.provider, modelPathFor(model), {
    inputs: text,
...
async function runCortex(text, timeoutMs = 25000) {
  ...
  const result = await hfChatCompletion(modelPathFor(model), messages, timeoutMs);
  if (!result.ok) {
...
async function runLinkModel(modelKey, normalizedUrl, ruleAnalysis, contextText = '', timeoutMs = 25000) {
...
async function runLinkDetection({
  ...
  const modelOrder = allowProviderFallback && requestedKey === 'sentinel' ? ['sentinel', 'swift'] : [requestedKey];
  const modelRuns = [];
  let lastError = null;
  ...
  for (const key of modelOrder) {
    ...
    try {
      const payload = await runLinkModel(key, validated.url, ruleAnalysis, validated.context, timeoutMs);
      if (key !== requestedKey) {
...
async function runPhishingDetection({
  ...
  const modelOrder = allowProviderFallback ? [...new Set([modelKey, fallbackKey])] : [modelKey];
  const modelRuns = [];
  let lastError = null;
  ...
  for (const key of modelOrder) {
    const started = Date.now();
    try {
      const payload = key === 'mailguard' ? await runMailGuard(text, timeoutMs) : await runCortex(text, timeoutMs);
      modelRuns.push({
        model_key: key,
