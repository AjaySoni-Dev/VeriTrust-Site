...
async function runMailGuard(text, timeoutMs = 20000, modelContract = null) {
  ...
  const { legitimate, normalized, phishing } = normalizeMailGuard(scores, contract);
...
async function runLinkDetection({
  ...
  for (const key of modelOrder) {
    ...
    try {
      const payload = await runLinkModel(key, validated.url, ruleAnalysis, validated.context, timeoutMs, modelContract);
      modelRuns.push({
        model_key: key,
        provider: model.provider,
        provider_model: modelContract?.repository_id || key,
        provider_revision: modelContract?.revision_sha || null,
        status: 'completed',
        latency_ms: Date.now() - started,
      });
      return {
...
async function runPhishingDetection({
  ...
  for (const key of modelOrder) {
    const started = Date.now();
    try {
      const payload = key === 'mailguard' ? await runMailGuard(text, timeoutMs, modelContract) : await runCortex(text, timeoutMs, modelContract);
      modelRuns.push({
        model_key: key,
        provider: PHISHING_MODELS[key].provider,
        provider_model: modelContract?.repository_id || null,
        provider_revision: modelContract?.revision_sha || null,
        status: 'completed',
        latency_ms: Date.now() - started,
      });
