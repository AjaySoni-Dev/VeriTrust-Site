...
function classifierTextChunks(text, maximumCharacters = 1800, maximumChunks = 8) {
...
async function runMailGuard(text, timeoutMs = 20000, modelContract = null) {
  ...
  const result = await hfJsonInference(model.provider, contract.repository_id, {
    inputs: chunks.length === 1 ? chunks[0] : chunks,
    parameters: {
  ...
  if (!result.ok) {
    throw hfInferenceError('Phishing classifier failed.', result);
  }

  const outputs = chunks.length === 1
    ? [result.json]
    : (Array.isArray(result.json) && result.json.length === chunks.length ? result.json : null);
  if (!outputs) throw new HttpError(502, 'The phishing classifier returned an invalid batch response.', { code: 'MODEL_OUTPUT_BATCH_MISMATCH' });
...
function splitLinkBatchOutput(output, expectedCount) {
  if (expectedCount === 1) return [output];
  if (!Array.isArray(output) || output.length !== expectedCount || output.some((item) => !Array.isArray(item))) {
    throw new HttpError(502, 'The link classifier returned an invalid batch response.', { code: 'MODEL_OUTPUT_BATCH_MISMATCH' });
  }
  return output;
}

async function runLinkModelBatch(modelKey, validatedItems, ruleAnalyses, timeoutMs = 20000, modelContract = null) {
  const model = LINK_MODELS[modelKey] || LINK_MODELS[DEFAULT_LINK_MODEL];
  if (model.locked || model.comingSoon || !model.model_path_key || !model.provider) {
    throw new HttpError(400, `${model.name} is not available in this version.`, { code: 'INVALID_MODEL' });
  }
  const repositoryId = modelContract?.repository_id || modelPathFor(model);
  const result = await hfJsonInference(model.provider, repositoryId, {
    inputs: validatedItems.map((item) => item.url),
  }, timeoutMs, modelContract);
  if (!result.ok) throw hfInferenceError('Link model batch inference failed.', result);

  const outputs = splitLinkBatchOutput(result.json, validatedItems.length);
  const normalizer = modelKey === 'sentinel' ? normalizeSentinelOutput : normalizeSwiftOutput;
  return outputs.map((modelOutput, index) => {
    const normalized = normalizer(modelOutput);
    if (!normalized.scores.length) {
      throw new HttpError(502, 'The link classifier returned an unexpected batch item.', { code: 'MODEL_OUTPUT_INVALID' });
    }
    const modelMeta = { key: model.key, name: model.name, fallback_used: false };
    return {
      ok: true,
      type: 'link',
      model: modelMeta,
      result: buildLinkResult({
        url: validatedItems[index].url,
        context: validatedItems[index].context,
        modelKey,
        modelOutput,
        ruleAnalysis: ruleAnalyses[index],
        modelMeta,
      }),
      scores: normalized.scores,
    };
...
async function runLinkDetection({
  ...
  const error = lastError || new HttpError(502, 'Link analysis failed.', { code: 'MODEL_ERROR' });
  error.modelRuns = modelRuns;
  throw error;
}

async function runLinkDetectionBatch({
  items,
  modelKey = DEFAULT_LINK_MODEL,
  timeoutMs = 20000,
  modelContract = null,
  ...
  if (!Array.isArray(items) || !items.length) {
    throw new HttpError(400, 'At least one URL is required for batch analysis.', { code: 'INVALID_INPUT' });
  }
  const validatedItems = items.map((item) => validateLinkInput({
    url: item.url,
    text: item.text,
    context: item.context || '',
  }));
  const ruleAnalyses = validatedItems.map((validated) => {
    const analysis = analyzeUrlRules(validated.url, validated.context);
    analysis.extracted.urls_found = validated.urls_found.length ? validated.urls_found : analysis.extracted.urls_found;
    return analysis;
  });
  const started = Date.now();
  const model = LINK_MODELS[modelKey];
  try {
    const resolvedContract = modelContract || environmentModelContract(modelKey);
    const payloads = await runLinkModelBatch(modelKey, validatedItems, ruleAnalyses, timeoutMs, resolvedContract);
    return {
      payloads: payloads.map((payload, index) => finalizeLinkPayload(payload, {
        scanId: items[index].scanId || null,
        context: items[index].workspaceContext || null,
        createdAt: items[index].createdAt || createdAt,
      })),
      modelRuns: [{
        model_key: modelKey,
        provider: model.provider,
...
async function runPhishingDetection({
  ...
  const error = lastError || new HttpError(502, 'Phishing analysis failed.', { code: 'MODEL_ERROR' });
  error.modelRuns = modelRuns;
  throw error;
}
