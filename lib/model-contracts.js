...
function environmentModelContract(modelKey, options = {}) {
  ...
  if (!candidate) {
    ...
    throw new ModelContractError('MODEL_CONTRACT_UNRESOLVED', `${normalizedKey} is missing from HF_MODEL_CONTRACTS.`, { modelKey: normalizedKey, source: 'environment' });
  }
...
function resolveModelContract(modelKey, candidate, options = {}) {
  if (candidate) {
    try {
      return { contract: validateModelContract(modelKey, candidate, { source: options.source || 'database' }), source: options.source || 'database' };
    } catch (error) {
      if (!options.allowEnvironmentFallback) throw error;
    }
  }
  const contract = environmentModelContract(modelKey, { required: true });
  return { contract, source: 'environment' };
}

function modelContractReadiness(modelKey) {
  try {
    const contract = environmentModelContract(modelKey, { required: false });
    if (!contract) return { ready: false, code: 'MODEL_CONTRACT_UNRESOLVED' };
    return {
      ready: true,
