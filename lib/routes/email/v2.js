...
const { validatePlainTextInput, validateReceiverEvent, validateTrustedReceiverMetadata, MAX_RAW_EML_BYTES } = require('../../email/contracts');
...
async function analyzeText(req, onProgress = () => {}) {
  ...
  return analyzeEmail({ auth, mode: 'plain_text', text, integrationId: text.integration_id, idempotencyKey, requestId: ids.requestId, traceId: ids.traceId, onProgress });
}
...
async function analyzeEml(req, onProgress = () => {}) {
  ...
  return analyzeEmail({ auth, mode: 'raw_eml', raw, idempotencyKey, requestId: ids.requestId, traceId: ids.traceId, onProgress });
}
...
module.exports = { analyzeEml, analyzeText, evidence, readRawBuffer, receiverEml, receiverEvent, trustedReceiverMetadata };
