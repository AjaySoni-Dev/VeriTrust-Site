...
const { fetchWithPolicy, parseCookies } = require('./security');
...
function bearerToken(req) {
  ...
  return parseCookies(req).vt_access || null;
}

async function readResponse(response, rawText = null) {
  const raw = rawText === null ? await response.text() : rawText;
  let data = null;
  try {
...
async function supabaseFetch(path, options = {}) {
  ...
  const { response, text } = await fetchWithPolicy(`${supabaseUrl()}${path}`, {
...
async function getUserFromRequest(req) {
  ...
  const sessionId = parseCookies(req).vt_session_id;
  if (sessionId && isServiceRoleConfigured()) {
    const sessionHash = crypto.createHash('sha256').update(sessionId).digest('hex');
    const rows = await supabaseFetch(`/rest/v1/user_sessions?session_hash=eq.${eq(sessionHash)}&user_id=eq.${eq(user.id)}&revoked_at=is.null&select=id&limit=1`, { service: true });
    if (!rows?.length) throw new SupabaseError(401, 'Session was revoked.', null, 'SESSION_REVOKED');
    await supabaseFetch(`/rest/v1/user_sessions?session_hash=eq.${eq(sessionHash)}`, { method: 'PATCH', service: true, body: { last_seen_at: new Date().toISOString() } }).catch(() => null);
  }
...
async function createScanRecord(context, options) {
  const retainText = options.retainText === true;
  return rpc('create_scan_record', {
    ...
    target_text_preview: retainText ? (options.textPreview || null) : null,
...
async function completeScanRecord(scanId, payload, modelRuns = []) {
  ...
  return rpc('complete_scan_record', body, { service: true });
}
...
async function recentScans(context, limit = 20) {
  ...
  return supabaseFetch(`/rest/v1/scans?org_id=eq.${eq(context.organization.id)}&select=id,scan_type,status,selected_model_key,final_label,confidence,risk_level,created_at,completed_at,error_message,scan_inputs(input_kind,text_preview,metadata),scan_results(label,confidence,risk_level,primary_score,secondary_score,explanation,indicators,raw_scores),scan_model_runs(model_key,provider,provider_model,status,latency_ms,error_message,created_at)&order=created_at.desc&limit=${safeLimit}`, {
    accessToken: context.token,
  });
}
...
async function scanTypeTotals(context) {
  ...
  for (const row of rows || []) {
    ...
    if (row.scan_type === 'link') totals.link_count += 1;
...
async function workspaceStats(context) {
  ...
  try {
    ...
    stats.usage_today.link_count = totals.link_count;
