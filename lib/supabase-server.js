...
async function scanTypeTotals(context) {
  const rows = await supabaseFetch(`/rest/v1/scans?org_id=eq.${eq(context.organization.id)}&user_id=eq.${eq(context.user.id)}&status=eq.completed&select=scan_type,metadata,scan_inputs(input_kind,metadata)`, {
    accessToken: context.token,
  });
  const totals = {
    deepfake_count: 0,
  ...
  for (const row of rows || []) {
    const scanType = logicalScanType(row);
    if (scanType === 'deepfake') totals.deepfake_count += 1;
    if (scanType === 'link') totals.link_count += 1;
    if (scanType === 'phishing') totals.phishing_count += 1;
  }
