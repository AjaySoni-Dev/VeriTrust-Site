...
async function handleStripeWebhook(req) {
  ...
  const prior = await supabaseFetch(`/rest/v1/billing_events?provider=eq.${PROVIDER}&event_id=eq.${eq(event.id)}&status=eq.processed&select=id&limit=1`, {
    service: true,
  });
  if (prior?.length) return { received: true, type: event.type, duplicate: true };
