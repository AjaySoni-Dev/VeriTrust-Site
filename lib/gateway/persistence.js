...
async function modelVersion(orgId, modelKey, artifactType) {
  ...
  const ordered = [...(rows || [])].sort((left, right) => Number(right.org_id === orgId) - Number(left.org_id === orgId));
...
function qualifiedModelConfiguration(version) {
  ...
  if (!configuration || configuration.registry_schema !== 'gateway-model-registry-2') {
    throw new HttpError(503, `Model registry metadata is unresolved for ${version?.model_key || 'the requested model'}.`, { code: 'GATEWAY_MODEL_CONTRACT_UNRESOLVED' });
  }
  if (!configuration.repository_id || !/^[0-9a-f]{40,64}$/iu.test(String(configuration.revision_sha || ''))) {
    throw new HttpError(503, `Immutable model identity is unresolved for ${version.model_key}.`, { code: 'GATEWAY_MODEL_IDENTITY_UNRESOLVED' });
  }
  if (!['production', 'canary'].includes(String(configuration.qualification_state || '').toLowerCase())) {
    throw new HttpError(503, `Model ${version.model_key} is not qualified for policy influence.`, { code: 'GATEWAY_MODEL_NOT_QUALIFIED' });
  }
...
async function prepareModelRun(orgId, scanId, artifact, modelKey) {
  ...
  const configuration = qualifiedModelConfiguration(version);
  const rows = await supabaseFetch('/rest/v1/gateway_model_runs?select=*', {
    ...
    body: {
      ...
      metrics: { registry_resolved_before_inference: true, registry_schema: configuration.registry_schema },
    },
    headers: { Prefer: 'return=representation' },
  });
  return { run: rows?.[0], version, configuration };
}
