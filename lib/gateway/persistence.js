...
async function authenticate(req, requiredAction) {
  ...
  if (!token) throw new HttpError(401, 'Authorization bearer token is required.', { code: 'UNAUTHORIZED' });

  if (token.startsWith('vtg_')) {
...
async function recordEvidence(orgId, scanId, artifact, evidence) {
  const version = await modelVersion(orgId, evidence.modelKey, artifact.type);
  const runRows = await supabaseFetch('/rest/v1/gateway_model_runs?select=*', {
    method: 'POST',
    service: true,
    body: {
      org_id: orgId,
      ...
      provider_model_version: version.version,
      calibration_version: version.calibration_version,
      preprocessing_version: version.preprocessing_version,
      latency_ms: Number.isFinite(Number(evidence.latencyMs)) ? Math.max(0, Math.round(evidence.latencyMs)) : null,
      error_code: evidence.errorCode || null,
      error_detail: evidence.errorCode ? { error_code: evidence.errorCode } : {},
      metrics: { degraded: Boolean(evidence.degraded) },
    },
    headers: { Prefer: 'return=representation' },
  });
  const run = runRows?.[0];
