...
function verifyDispatchSignature(req, secret, dispatch, now = Date.now()) {
  const timestamp = String(req?.headers?.['x-veritrust-dispatch-timestamp'] || '').trim();
  const nonce = String(req?.headers?.['x-veritrust-dispatch-nonce'] || '').trim().toLowerCase();
  const signature = String(req?.headers?.['x-veritrust-dispatch-signature'] || '').trim().toLowerCase();
...
module.exports = { constantTimeEqual, parseDispatchPayload, signingMessage, verifyDispatchSignature };
