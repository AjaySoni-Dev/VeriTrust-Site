...
function adapterFor(kind) {
  if (kind === 'phishing') return phishingAdapter;
  if (kind === 'link') return linkAdapter;
  return null;
}
...
async function executeRoute(route, policy, prepared) {
  ...
  return adapter.execute(route.artifact, {
    modelKey: route.modelKey,
    required: route.required,
    timeoutMs: Math.min(policy.timeouts.per_model_ms, serverConfig.gatewaySynchronousBudgetMs),
...
async function orchestrateSubmission(input) {
  ...
  try {
    ...
    const preparedRoutes = await Promise.all(routesToRun.map(async (route) => ({
      route,
      prepared: await prepareModelRun(auth.organization.id, scanId, route.artifact, route.modelKey),
    })));
    const executionResults = await Promise.all(preparedRoutes.map(({ route, prepared }) => executeRoute(route, policy, prepared)));
    const newEvidence = await Promise.all(executionResults.map((result, index) => {
      const route = routesToRun[index];
      return recordEvidence(auth.organization.id, scanId, route.artifact, result.evidence, preparedRoutes[index].prepared);
    }));
