...
async function executeRoute(route, policy) {
...
async function orchestrateSubmission(input) {
  ...
  try {
    ...
    const executionResults = await Promise.all(routesToRun.map((route) => executeRoute(route, policy)));
    const newEvidence = await Promise.all(executionResults.map((result, index) => {
      const route = routesToRun[index];
      return recordEvidence(auth.organization.id, scanId, route.artifact, result.evidence);
    }));
