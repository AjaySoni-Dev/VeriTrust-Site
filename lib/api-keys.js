...
const DEFAULT_SCOPES = ['deepfake:scan', 'phishing:scan', 'link:scan', 'usage:read'];
...
async function requireApiKey(req, requiredScope) {
  ...
  const entitlement = await enforceEntitlement(auth, {
    action: requiredScope === 'usage:read' ? 'api_usage_read' : 'api_scan',
    source: 'api',
    scanType: requiredScope === 'phishing:scan'
      ? 'phishing'
