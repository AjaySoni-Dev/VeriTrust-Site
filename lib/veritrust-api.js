...
function isAllowedOrigin(req, origin) {
  ...
  if (normalizeOrigin(origin) === normalizeOrigin(requestOrigin(req))) return true;

  const allowed = serverConfig.allowedOrigins.map(normalizeOrigin);
...
async function hfChatCompletion(modelId, messages, timeoutMs = 30000) {
  const token = readHfToken();
  return hfRequest(
