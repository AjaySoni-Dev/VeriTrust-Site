...
const LINK_MODELS = {
  swift: {
    ...
    revision: String(process.env.VERITRUST_MODEL_SWIFT_REVISION || '').trim(),
    adapterVersion: 'swift-url-v1',
  },
  sentinel: {
    key: 'sentinel',
...
function validateLinkInput({ url, text, context } = {}) {
  ...
  if (urlsFound.length > MAX_URLS_PER_REQUEST) {
    throw linkValidationError('TOO_MANY_URLS', `Analyze at most ${MAX_URLS_PER_REQUEST} URLs per request.`);
  }
  const urlItems = urlsFound.map((item) => {
    const parsedItem = parseUrlInput(item);
    return parsedItem
      ? { input_url: item, normalized_url: parsedItem.normalized_url, ok: true }
      : { input_url: item, normalized_url: null, ok: false, error: { code: 'INVALID_URL', message: 'URL is malformed or unsupported.' } };
  });
  const firstUrl = urlItems.find((item) => item.ok)?.normalized_url || source;
  ...
  return {
    ...
    urls_found: urlItems.filter((item) => item.ok).map((item) => item.normalized_url),
    url_items: urlItems,
...
function strictModelScores(output, allowedLabels, adapterName) {
  const raw = flattenModelOutput(output);
  if (!raw.length) throw linkValidationError('MODEL_CONTRACT_ERROR', `${adapterName} returned no scores.`, 502);
  const seen = new Set();
  return raw.map((item) => {
    const label = String(item?.label ?? item?.name ?? '').trim().toLowerCase();
    const score = Number(item?.score ?? item?.value);
    if (!allowedLabels.has(label) || seen.has(label) || !Number.isFinite(score) || score < 0 || score > 1) {
      throw linkValidationError('MODEL_CONTRACT_ERROR', `${adapterName} returned an unknown or invalid label contract.`, 502);
    }
    seen.add(label);
    return scoreItem(label, score);
  });
}

function normalizeSwiftOutput(output) {
  const allowed = new Set(['benign', 'safe', 'label_0', 'phishing', 'label_1', 'malware', 'malicious', 'label_2', 'defacement', 'label_3']);
  const scores = strictModelScores(output, allowed, 'Swift');
  let benign = 0;
  let phishing = 0;
  let malware = 0;
  let defacement = 0;
  ...
  for (const item of scores) {
    ...
    if (['benign', 'safe', 'label_0'].includes(label)) benign = Math.max(benign, item.score);
    if (['phishing', 'label_1'].includes(label)) phishing = Math.max(phishing, item.score);
    if (['malware', 'malicious', 'label_2'].includes(label)) malware = Math.max(malware, item.score);
    if (['defacement', 'label_3'].includes(label)) defacement = Math.max(defacement, item.score);
  }
...
function normalizeSentinelOutput(output) {
  const allowed = new Set(['safe', 'legitimate', 'label_0', 'phishing', 'malicious', 'label_1']);
  const scores = strictModelScores(output, allowed, 'Sentinel');
  let safe = 0;
  let phishing = 0;

  for (const item of scores) {
    const label = String(item.label || '').toLowerCase();
    if (['safe', 'legitimate', 'label_0'].includes(label)) safe = Math.max(safe, item.score);
    if (['phishing', 'malicious', 'label_1'].includes(label)) phishing = Math.max(phishing, item.score);
  }
